...
export default function UpdateUserForm() {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (Object.keys(formData).length == 0) return("Don't have Form Data")
    ...
    return (
        <form className="grid lg:grid-cols-2 w-4/6 gap-4" onSubmit={handleSubmit}>
            ...
            <div className="input-type">
                <input type="date" onChange={setFormData} name='date' placeholder="Date" className="border px-5 py-3 focus:outline-none rounded-md"/>
            </div>
            ...
            <button className="flex justify-center text-md w-2/6 bg-orange-500 text-white px-4 py-2 border rounded-md hover:bg-gray-50 hover:border-orange-500 hover:text-orange-500">
            Update <span className="px-1 py-0.5"><BiEditAlt size={19}/></span>
            </button>
        </form>
    )
}